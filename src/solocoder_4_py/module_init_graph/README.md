...
当 Kahn 算法检测到环后，会进一步通过**基于 DFS 的环查找**找出所有简单环，生成 `CycleReport`：

- **`CycleReport.cycles`**：所有环的列表，每个环是模块 ID 的有序列表
- **`CycleReport.involved_modules()`**：所有涉及循环的模块集合
- **`CycleReport.format_report()`**：可读报告，形如：
  ```
  检测到 2 个循环依赖:
  ...
  ```
- 抛出的 `CircularDependencyError` 异常中携带 `cycles` 字段
...
重试模块 M:
  ...
  │   └─ 先检查上游是否已就绪；若未就绪直接返回（不执行）
  │
  ├─ 情况 2：M 是 FAILED / ISOLATED（上游就绪）
  │   ├─ 重置 M 的状态为 PENDING
  │   ├─ 执行初始化回调（含额外重试次数）
  │   │   ├─ 成功 → INITIALIZED，继续下一步
  │   │   └─ 失败 → FAILED，再次隔离所有下游
  │   │
  │   └─ 若 M 成功 → 级联恢复下游:
...
| 方式 | 设置位置 | 触发时机 | 用途 |
|------|---------|---------|------|
| 内置重试 `max_retries=N` | `ModuleNode(max_retries=N)` | 首次 `execute_init` 时自动重试 | 应对瞬时故障（网络抖动） |
| 局部重试 `extra_retries=N` | `retry_module(extra_retries=N)` | 手动调用时 | 修复故障后手动触发恢复 |
...
| 异常类型 | 触发场景 |
|---------|---------|
| `CircularDependencyError` | 依赖图存在环（异常中带 `cycles` 字段） |
...
| `RetryLimitExceededError` | （保留）重试次数超过限制 |
